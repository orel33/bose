...
Le bouton 3 des trois enceintes a été réaffecté à RTL2, à la place de la playlist
Deezer. Le bridge doit fonctionner pour lancer les trois radios avec les boutons.
Les presets natifs 1 et 2 ont également été alignés sur les URL de France Inter
et France Info du bridge : Veranda contenait auparavant Radio Bollerwagen sur 1.
Conserver cet alignement pour éviter deux affectations différentes d'un bouton.
...
**Oui, pour les radios sur 1/2/3 et la bascule du groupe sur 6.**
Docker maintient le bridge en arrière-plan, même après fermeture du terminal.
Si le bridge s'arrête, une lecture déjà lancée peut continuer. Le groupe Bose
...
5. Sur le Pi, avec les Bose accessibles :
   ...
   Si le bouton **6 de Veranda** ne provoque aucun message `[group] bouton 6 reçu`
   alors que le bouton 1 est bien détecté, débrancher puis rebrancher **Veranda**
   après la configuration du repère 6. Lors du test sur ces SoundTouch 10, le
   repère était visible dans l'API mais les appuis physiques sur 6 n'ont été
   transmis qu'après ce redémarrage. Attendre sa reconnexion `ws connected`,
   choisir 1, 2 ou 3, puis réessayer un appui court sur 6.
...
python3 tools/bose.py key 3                          # Simuler le bouton 3 RTL2 (bridge requis)
...
Pour les radios UPnP, la reprise envoie explicitement `Stop`, le flux précédent
via `SetAVTransportURI`, puis `Play`, avant de basculer le groupe.
...
Après redémarrage de Veranda, deux appuis physiques successifs sur 6 ont été reçus,
avec création puis séparation du groupe confirmées par l'API et reprise de France
Inter sur Veranda. La lecture audible sur les secondaires lors de ce test reste
à confirmer ; l'état de groupe dans l'API ne suffit pas à garantir le son.
Le comportement des boutons des secondaires en groupe reste à valider sur le matériel.
